...
var _Formatter = require("./Formatter.mjs");

var _walk = require("./walk.mjs");

function format(ast, options = {}) {
  const fmt = new _Formatter.Formatter(options);
...
const Precedence = {
  ...
  // `x.y` `x[y]` `x(...)` `new x(...)`
  NoArgNew: 2,
  // `new x` (right associative)
  Postfix: 3,
  // `x++` `x--`
  Prefix: 4,
  // `!x` `~x` `+x` `-x` `++x` `--x` `typeof x` `void x` `delete x` `await x` (right associative)
  Exponent: 5,
  ...
  // `x << y` `x >> y` `x >>> y`
  Compare: 9,
  // `x < y` `x <= y` `x > y` `x >= y` `x in y` `x instanceof y`
  Equal: 10,
  // `x == y` `x != y` `x === y` `x !== y`
  BitAnd: 11,
  // `x & y`
  ...
  Assignment: 17,
  // `x = y` `x += y` `x -= y` `x **= y` `x *= y` `x /= y` `x %= y` `x <<= y` `x >>= y` `x >>>= y` `x &= y` `x ^= y` `x |= y` (right associative)
  Yield: 18,
  // `yield x` `yield* x` (right associative)
  Sequence: 19 // `x, y`
