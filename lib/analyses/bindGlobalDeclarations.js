...
exports.bindGlobalDeclarations = bindGlobalDeclarations;
...
function bindGlobalDeclarations(scriptAst, execContext) {
  ...
  (0, _walk.walk)(scriptAst, {
    ...
    Pattern(node, state) {
      ...
      if (node.type === 'Identifier' && decl != null) {
        ...
        if (envRec.hasVarDeclaration(name) || lexical && (envRec.hasLexicalDeclaration(name) || envRec.hasRestrictedGlobalProperty(name) || !newName)) {
  ...
  while (funcDecl = funcsToInit.shift()) {
    const name = funcDecl.id.name;
    const model = (0, _function.newFunction)(execContext.realm, env, funcDecl, execContext.strict);
