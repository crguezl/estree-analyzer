...
exports.bindFunctionDeclarations = bindFunctionDeclarations;

var _walk = require("../walk.mjs");

var _environment = require("../model/environment.mjs");

var _function = require("../model/function.mjs");
...
function bindFunctionDeclarations(func, args, execContext) {
  ...
  for (const paramNode of func.formalParameters) {
    (0, _walk.walk)(paramNode, undefined, {
      Pattern(node) {
  ...
  const argumentsObjectNeeded = func.thisMode !== _function.ThisMode.Lexical && !parameterNames.has(argumentsName) && !hasParameterExpressions && !declaredNames.has(argumentsName); // bind parameters
  ...
  if (!hasParameterExpressions) {
    ...
    varEnv = (0, _environment.newDeclarativeEnvironment)(env);
  ...
  const lexEnv = strict ? varEnv : (0, _environment.newDeclarativeEnvironment)(varEnv);
  const lexEnvRec = lexEnv.record;
  execContext.lexicalEnvironment = lexEnv; // bind lexicals
  ...
  for (const funcDecl of funcsToInit) {
    const name = funcDecl.id.name;
    const model = (0, _function.newFunction)(execContext.realm, lexEnv, funcDecl, strict);
