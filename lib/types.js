...
function hasKind(type, kind) {
  return Array.isArray(type) && type !== 'union' ? type.some(t => getKind(t) === kind) : getKind(type) === kind;
}
/**
...
function isFalsy(type) {
  return Array.isArray(type) ? type.length > 0 && type.every(isFalsy) : ['undefined', 'null', 'void'].includes(getKind(type));
}
/**
...
function isTruthy(type) {
  return Array.isArray(type) ? type.length > 0 && type.every(isTruthy) : ['symbol', 'object', 'function', 'array'].includes(getKind(type));
}
/**
...
function isAssignable(target, source) {
  ...
  switch (targetKind) {
    ...
    case 'function':
      return sourceKind === targetKind && isAssignable(target.returns, source.returns) && !!target.params && !!source.params && target.params.length === source.params.length && source.params.every((p, i) => isAssignable(p.type, target.params[i].type));
  ...
  if (a && b) {
    ...
    if (isUnion(a)) {
      if (isUnion(b)) {
        ...
        const members = new Set(aTypes); // compare pairwise assignability of elements of `a` and `b`

        for (const be of getUnionTypes(b)) {
          for (const ae of aTypes) {
            if (isAssignable(be, ae)) {
              if (isAssignable(ae, be)) {// equivalent: ignore `be`
              } else {
                // `be` is supertype of `ae`: replace `ae` with `be`
                members.delete(ae);
                members.add(be);
              }
            } else if (isAssignable(ae, be)) {// `ae` is supertype of `be`: ignore `be`
            } else {
...
function formatParam(param) {
  ...
  return param.name && param.type ? param.name + ': ' + paramType : param.type ? ':' + paramType : param.name || '_';
}
/**
