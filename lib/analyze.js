 ...
 * @alias analyze
 */
function analyze(ast, rootScope = new _scope.Scope()) {
...
const visitors = {
  Identifier(ast, scope, declContext) {
    ...
    if (!ref) {
      if (declContext === 'var') {
        ...
        scope = scope.getRoot();
      } else {// const, let, and function/catch parameters go in given scope
      }
  ...
  Literal(ast) {
    ...
    const type = (0, _types.kindOf)(value); // null, string, boolean, number, object (RegExp)
  ...
  EmptyStatement() {// ignored
  },

  DebuggerStatement() {// ignored
  },
  ...
  BreakStatement() {// ignored
  },

  ContinueStatement() {// ignored
  },
  ...
  ArrayExpression(ast, scope) {
    ...
    for (const element of ast.elements) {
      ...
      if (elemInfo && 'type' in elemInfo && elemType !== null) {
        if (elemType === undefined) {
          ...
          elemType = (0, _types.union)(elemType, elemInfo.type);
        }
      } else {
        elemType = null;
      }
    ...
    const result = {
      type: (0, _types.arrayOf)(elemType)
    };
      ...
      case '+':
        {
          ...
          const type = leftType === 'string' || rightType === 'string' ? 'string' : (0, _types.isNotAssignable)(leftType, 'string') && (0, _types.isNotAssignable)(rightType, 'string') ? 'number' : undefined;
  ...
  LogicalExpression(ast, scope) {
    ...
    switch (ast.operator) {
      case '||':
        evaluate((l, r) => l || r, l => 'value' in l ? !!l.value : (0, _types.isTruthy)(l.type));
        break;

      case '&&':
        evaluate((l, r) => l && r, l => 'value' in l ? !l.value : (0, _types.isFalsy)(l.type));
    }
  ...
  MemberExpression(ast, scope) {
    let memberInfo;
    const objInfo = visit(ast.object, scope) || {}; // assume object is an 'object' unless we already know it is an 'array'

    const objType = 'object';

    if (!objInfo.type) {
      objInfo.type = objType;
    } else if (!(0, _types.hasKind)(objInfo.type, 'array')) {
      objInfo.type = (0, _types.union)(objInfo.type, objType);
    }
    ...
    if (!ast.computed) {
      ...
      memberInfo = visit(ast.property, _scope.Scope.withMembers(members));
    } else {
...
function analyzeBody(ast, scope) {
  ...
  for (const stmt of ast.body) {
    if (processDirectives) {
      if (stmt.type === 'ExpressionStatement' && stmt.expression.type === 'Literal' && stmt.expression.value === 'use strict') {
        scope.useStrict();
        continue;
      }
...
function unionInfo(a, b) {
  if (a && b) {
    ...
    if (a.type && b.type) {
      result.type = (0, _types.union)(a.type, b.type);
    }
